...
async function authenticate(transaction: string) {
  ...
  if (response.entries.length) {
    const entry = await ledger.account({ accountId: account as `G${string}` });
...
async function handle(request: Request): Promise<Response> {
  ...
  if (request.method === "GET" && url.pathname === "/auth") {
    ...
    return Response.json({ transaction, network_passphrase: Networks.TESTNET });
  }
...
console.log(
  `Local Testnet SEP-10 fixture: ${endpoint}. Tokens expire after two minutes. Ctrl-C stops it.`,
);
